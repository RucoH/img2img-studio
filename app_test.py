...
# Main generation function with auto-optimized params
def generate(
    image: Image.Image,
    prompt: str,
    style: str,
    ...
    # Auto-optimize strength and guidance
    strength = 0.8
    guidance_scale = 7.0
    ...
    for i in range(n_samples):
        logs.append(f"Generating sample {i+1}/{n_samples} with strength={strength}, guidance={guidance_scale}...")
        out = pipe(
            prompt=full_prompt,
            image=img,
            strength=strength,
...
with gr.Blocks(theme=theme) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            n_samples = gr.Slider(1, 4, value=1, step=1, label="Number of samples")
